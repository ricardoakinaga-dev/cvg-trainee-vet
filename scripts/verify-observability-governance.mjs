...
export function validateObservabilityGovernance(snapshot) {
  ...
  const signals = Array.isArray(policy.signals) ? policy.signals : [];
  ...
  for (const signal of signals) {
    ...
    if (
      typeof signal?.escalation !== "string" ||
      signal.escalation.trim() === ""
    )
    ...
    if (
      ...
      errors.push(
        `observability signal ${id} acknowledgement window is invalid`,
      );
    }
  ...
  if (alerts.length !== REQUIRED_SIGNAL_IDS.length) {
    errors.push(
      `observability policy must contain ${REQUIRED_SIGNAL_IDS.length} alerts`,
    );
  }
  for (const name of duplicateIds(alertNames)) {
    errors.push(`observability alert ${name} is duplicated`);
  }
  const knownSignalIds = new Set(REQUIRED_SIGNAL_IDS);
  for (const alert of alerts) {
    ...
    if (!knownSignalIds.has(alert?.signalId))
      errors.push(`observability alert ${name} references an unknown signal`);
    if (!new Set(["warning", "critical"]).has(alert?.severity))
