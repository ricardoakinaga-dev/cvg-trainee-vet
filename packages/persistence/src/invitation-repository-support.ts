...
): InvitationTransactionalOperations["account"] {
  return Object.freeze({
    ...
    setPassword: async (
      ...
      const updated = await executor
        ...
        .where(and(eq(accounts.id, accountId), eq(accounts.status, "INVITED")))
        .returning({ id: accounts.id });
      if (updated.length === 0) {
        throw new PersistenceConflictError(
          "invited account is already active or unavailable",
        );
      }
    },
...
function createInvitationPort(
  executor: DatabaseExecutor,
): InvitationTransactionalOperations["invitation"] {
  return Object.freeze({
...
export function createInvitationOperations(
  ...
  return Object.freeze({
    account: createAccountOperations(executor),
    invitation: createInvitationPort(executor),
