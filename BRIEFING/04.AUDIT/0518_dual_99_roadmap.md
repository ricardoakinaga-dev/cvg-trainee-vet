...
Código/teste `48e1014` e documentação inicial `abb1657` foram publicados.
...
findings `staged:*` sem fallback `staged:<git>`; GREEN desconta
`MAX_SCAN_BYTES + 1` em cada output-cap abortado e falha fechado ao esgotar
`256 MiB`, descartando findings parciais.

O foco passou `64/64`, a cobertura passou `205/1160/21` em
