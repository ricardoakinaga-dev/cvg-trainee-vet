...
| Persistência | `packages/persistence/src/journey-repository.ts`, `activity-repository.ts`, `attempt-repository.ts` | mapper validado, módulo/proveniência nulos fail-safe, todos os itens/versões `PUBLICADO` e início fechado para conteúdo não publicado |
...
proveniência de assignment implícita. A implementação corrigiu todos antes da
segunda revisão. A crítica independente final (Bacon) aprovou localmente e
registrou três P2: rederivação defensiva no boundary, E2E clicando a nova
tentativa e limpeza da justificativa. O commit `c16c52e` fechou esses três
pontos; nenhum P0/P1 funcional permaneceu localmente.

### GREEN e regressão

- focal pós-hardening: HTTP boundary final 68/68 e E2E de nova tentativa 1/1
  passaram; a suíte focal anterior de aplicação/persistência/contrato havia
  passado com 6 arquivos / 100 testes;
- `pnpm verify`: 131 arquivos / 632 testes passaram, 27 arquivos / 33 testes
  ignorados;
- cobertura global: 84,92% statements, 81,13% branches, 86,46% functions e
  85,67% lines;
- `pnpm build`: 12 workspaces passaram no rebuild final;
...
revisão clínica. A ausência de banco não é convertida em PASS.
