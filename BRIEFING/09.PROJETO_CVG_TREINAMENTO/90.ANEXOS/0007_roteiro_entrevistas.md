# Anexo 0007 — Roteiro de Entrevistas e Levantamento Operacional

**Projeto:** Sistema CVG de Treinamento Veterinário  
**Data:** 2026-08-05  
**Objetivo:** instrumento de trabalho para resolver os bloqueios B-01 (fluxo atual), B-02 (público e coorte), B-06 (áreas do piloto) e B-07 (baseline) antes da nova submissão do gate Discovery e, somente depois de sua aprovação, do gate PRD.
**Uso:** entrevistador (PO ou coordenação educacional indicada) aplica e registra em formulário próprio; nenhuma informação nova é inventada — tudo o que não for confirmado permanece `PENDENTE`.

> **Pré-condição LGPD (B-05):** até a validação formal da política de dados e a nomeação do responsável, este anexo pode ser usado apenas para desenhar instrumentos e obter informações estritamente agregadas e anônimas. Ficam bloqueados formulários individualizados, nomes, cargo/turno vinculáveis, gravações, diagnóstico individual, prontuários, casos reais e indicadores assistenciais vinculáveis. B-05 precede B-07.

---

## 1. Quem entrevistar e por quê

| Grupo | Papel | Objetivo da entrevista | Bloqueios atendidos |
|---|---|---|---|
| A — Direção / patrocinador | MV. Ricardo Akinaga (CEO) | finalidade do programa, orçamento, tempo protegido, política de uso dos dados | B-02, B-03, B-05, B-06 |
| B — Coordenação clínica / RT | coordenação clínica ou responsável técnico | fluxo atual, lacunas clínicas, protocolos, áreas do piloto, temas críticos | B-01, B-06, B-07 |
| C — Coordenação educacional | quem hoje organiza capacitação (ou candidato ao papel) | processo atual de treinamento, materiais, avaliações, carga, registros | B-01, B-03, B-07 |
| D — Veterinários colaboradores | amostra de 5 a 8 ou proporcional por área/turno | como estudam hoje, tempo disponível, dispositivos, motivação, percepção de lacunas | B-01, B-02, B-07 |

**Ordem sugerida:** A → B → C → D (a direção fornece o contexto que orienta as demais; a coordenação prepara as perguntas específicas para os veterinários).

**Logística por entrevista:**
- Duração: 30–45 min (A, B, C); 15–25 min (D);
- Presencial ou remoto, conforme rotina de plantão (escala 12×36);
- Gravação bloqueada até B-05; depois, somente se a política validada permitir e houver base legal/consentimento aplicável;
- Preencher um [Formulário de registro](#7-formulario-de-registro-de-entrevista) por entrevistado;
- Após todas: consolidar em documento único e validar com os participantes (sessão de devolutiva).

---

## 2. Roteiro A — Direção / patrocinador (30–45 min)

**Foco:** finalidade, patrocínio, governança, dados, tempo protegido.

| # | Pergunta | Informação que busca | Decisão/bloqueio |
|---|---|---|---|
| A1 | Qual é a finalidade principal do programa: desenvolvimento, conformidade, certificação interna ou combinação? | finalidade oficial (define regras de uso) | D-049, política de dados |
| A2 | Quantos veterinários o CVG tem hoje, em quais setores e turnos? | dimensionamento | B-02 |
| A3 | Como os treinamentos são realizados hoje? Existe algo além do que já sabemos (que não há sistema)? | fluxo real | B-01 |
| A4 | Há tempo protegido para estudo? Quanto por semana/mês por veterinário? | carga viável | D-016 |
| A5 | Quais são os titulares e suplentes de PO, responsável clínico/RT, coordenação educacional, comitê científico e LGPD/segurança, e onde estão registrados seus aceites e conflitos? | nomeações conforme D-071/0006 | B-03 |
| A6 | Quem poderá ver resultados individuais? Gestão de pessoas pode usar para RH? | política de acesso | D-051, D-052 |
| A7 | Há licença institucional das três obras (Tratado, Ettinger e Fossum) ou condição de uso definida? | direitos | B-04 |
| A8 | Qual orçamento e prazo desejado para o piloto? | viabilidade | D-065, D-066 |
| A9 | Qual coorte e duração do piloto prefere? | recorte do piloto | D-060, D-061 |
| A10 | Quais três lacunas mais preocupam a direção? | prioridades | B-06 |

---

## 3. Roteiro B — Coordenação clínica / RT (30–45 min)

**Foco:** fluxo clínico atual, lacunas, protocolos, áreas do piloto, temas críticos e baseline.

| # | Pergunta | Informação que busca | Decisão/bloqueio |
|---|---|---|---|
| B1 | Descreva o fluxo real de capacitação hoje, do início ao fim: quem faz o quê? | mapa do fluxo | B-01 |
| B2 | Como os temas de treinamento são escolhidos hoje? | critérios atuais | B-01 |
| B3 | Quem cria ou seleciona materiais? Quem aprova? | papéis reais | B-01, B-03 |
| B4 | Como presença, conclusão e nota são registradas hoje? | ferramentas/registros | B-01 |
| B5 | Quais três lacunas clínicas mais preocupam a coordenação? | priorização de conteúdo | B-06 |
| B6 | Quais temas têm maior risco e frequência na operação? | escopo do piloto | B-06 |
| B7 | Quais protocolos internos existem hoje? Onde estão documentados? | inventário de protocolos | D-025 |
| B8 | Quais temas exigem reciclagem obrigatória? | regra de validade | D-026 |
| B9 | Como uma divergência entre livro, protocolo interno e diretriz atual é tratada hoje? | processo de decisão clínica | D-025, anexo 0001 |
| B10 | Existem temas recorrentes que possam inspirar cenários totalmente fictícios, sem acessar ou registrar casos reais? | insumo de conteúdo seguro | segurança/B-05 |
| B11 | Emergência e internação são boas áreas para o primeiro piloto? Quais seriam? | validação do piloto | B-06, D-020 |
| B12 | Quais indicadores clínicos existem hoje que podem servir de contexto (sem correlação causal)? | baseline de indicadores | B-07, D-056 |
| B13 | Qual é a composição da equipe (experiência, tempo de formação, tempo no CVG)? | segmentação | B-02 |
| B14 | Quais comportamentos você espera ver mudados com o treinamento? | critérios de sucesso | D-050 |

---

## 4. Roteiro C — Coordenação educacional (30–45 min)

**Foco:** processo educacional atual, materiais, avaliações, carga e registros.

| # | Pergunta | Informação que busca | Decisão/bloqueio |
|---|---|---|---|
| C1 | Existem atividades isoladas de treinamento hoje? Descreva cada uma. | práticas isoladas | B-01 |
| C2 | Que ferramentas são usadas (planilhas, grupos de mensagem, vídeo, pastas)? | inventário de ferramentas | B-01 |
| C3 | Existem provas, quizzes ou estudos de caso? Como são corrigidos? | avaliação atual | B-01 |
| C4 | Como ocorre a recuperação/reforço hoje? | remediação atual | B-01 |
| C5 | Como são tratadas contestações de nota hoje? | processo de contestação | D-046 |
| C6 | O que acontece quando um conteúdo fica desatualizado? | manutenção | D-035 |
| C7 | Como novos veterinários são nivelados na entrada? | onboarding atual | B-01 |
| C8 | Quanto tempo de coordenação seria aceitável dedicar ao programa por semana? | carga operacional | D-016 |
| C9 | Quais módulos/temas deveriam compor o núcleo obrigatório do piloto? | núcleo | D-021 |
| C10 | Qual carga mensal por veterinário é realista (proposta: 2–4 h/mês)? | cadência | D-016 |
| C11 | Quantos itens por objetivo são viáveis de produzir/revisar no piloto? | banco de questões | D-063 |
| C12 | Há necessidades de acessibilidade ou restrição de dispositivo/conectividade? | restrições | D-017, D-018 |

---

## 5. Roteiro D — Veterinários colaboradores (15–25 min)

**Foco:** rotina real, hábitos de estudo, tempo, dispositivos, motivação e percepção.

**Seleção da amostra:** 5 a 8 veterinários cobrindo diferentes setores, turnos e níveis de experiência; garantir pelo menos um de cada área proposta para o piloto.

| # | Pergunta | Informação que busca | Bloqueio |
|---|---|---|---|
| D1 | Como você estuda ou se atualiza hoje (livros, cursos, colegas, internet)? | hábito real | B-01 |
| D2 | Quanto tempo você conseguiria dedicar por semana, na rotina 12×36? | carga viável | D-016 |
| D3 | Onde você estudaria (casa, hospital, celular, computador)? Conectividade? | restrições | D-017 |
| D4 | Em quais temas você se sente menos seguro no dia a dia? | lacunas percebidas | B-06 |
| D5 | Você já participou de algum treinamento no CVG? Como foi? | experiência real | B-01 |
| D6 | O que faria você usar uma plataforma de treinamento de verdade? | motivação/adesão | R-02 |
| D7 | Como você se sentiria fazendo uma avaliação diagnóstica no início (não punitiva)? | aceitação | R-10 |
| D8 | O que você acha de ter seus resultados de estudo vistos por gestores? | política de dados | D-051 |
| D9 | Que formato funciona melhor para você: quiz curto, caso clínico, leitura, vídeo? | formato | D-023 |
| D10 | Alguma barreira de acessibilidade ou preferência de formato? | acessibilidade | D-018 |
| D11 | Você sabe quando um conteúdo do CVG é atualizado? Como fica sabendo? | comunicação | B-01 |
| D12 | O que você esperaria de um mentor? | mentoria | D-004 |

---

## 6. Inventário de ferramentas e materiais (checklist a verificar)

Para cada item, registrar: existe? qual? onde está? quem usa? serve de insumo?

- [ ] Materiais didáticos existentes (apostilas, slides, resumos);
- [ ] Pastas/drive com materiais;
- [ ] Grupos de mensagem usados para treinamento;
- [ ] Planilhas de presença/notas;
- [ ] Sistema de RH (registro de treinamentos);
- [ ] Existência de sistema de gestão clínica/prontuário, registrada somente de forma agregada; acesso ao sistema ou aos prontuários bloqueado por B-05;
- [ ] Ferramentas de videoconferência;
- [ ] Processo de aplicação de provas (formato, correção);
- [ ] Registro de certificados;
- [ ] Processo de revisão clínica (reuniões, atas);
- [ ] Lista de cursos externos usados/patrocinados;
- [ ] Protocolos internos documentados (levantar todos);
- [ ] Existência de categorias de indicadores assistenciais, sem valores ou vínculos individuais; coleta bloqueada por B-05.

---

## 7. Formulário de registro de entrevista

**Preencher um por entrevista.**

```text
ENTREVISTA: ___/___/2026
GRUPO (A/B/C/D): ___
NOME (se autorizado): ____________________   CARGO/ÁREA: ____________
TURNO: ____________   TEMPO NO CVG: ____________   TEMPO DE FORMAÇÃO: ____________

CONSENTIMENTO DE GRAVAÇÃO/REGISTRO: [ ] sim  [ ] não  (documentar anexo)

PERGUNTAS E RESPOSTAS (resumo literal, sem interpretação):
#1: ...
#2: ...

OBSERVAÇÕES DO ENTREVISTADOR (não confundir com resposta):
- ...

CLASSIFICAÇÃO DAS INFORMAÇÕES (marcar por trecho):
[ ] FATO INFORMADO   [ ] EVIDÊNCIA DOCUMENTAL   [ ] HIPÓTESE   [ ] PENDENTE

LINKS PARA DECISÕES: D-___, B-___
```

---

## 8. Saídas esperadas e como alimentam o briefing

| Saída | Documento que atualiza | Gate afetado |
|---|---|---|
| Mapa do fluxo atual validado (etapas, atores, ferramentas, gargalos, exceções) | `00.DISCOVERY/0003_fluxo_atual.md` | B-01 |
| Inventário de ferramentas e materiais | `00.DISCOVERY/0002_contexto_operacional.md` | B-01 |
| Quantidade e perfil dos veterinários (segmentação) | `00.DISCOVERY/0006_usuarios_e_stakeholders.md` | B-02 |
| Lacunas prioritárias e áreas do piloto confirmadas | `00.DISCOVERY/0005_hipotese_de_valor.md`, `01.PRD/0011` | B-06 |
| Linha de base mínima (perfil, conhecimento, engajamento, indicadores) | `01.PRD/0015_metricas_de_sucesso.md` | B-07 |
| Nomeações de responsáveis | `00.DISCOVERY/0006`, `01.PRD/0012 §8` | B-03 |
| Política de dados definida | `01.PRD/0014`, `0012` | B-05 |
| Situação de licenças | `90.ANEXOS/0001` | B-04 |
| Respostas das 12 perguntas de `0005_decisoes_pendentes.md §8` | `90.ANEXOS/0005` | geral |

---

## 9. Critérios de fechamento (o que comprova o fim do levantamento)

O levantamento estará concluído quando:

1. Fluxo atual desenhado, com etapas, atores, ferramentas, gargalos, falhas e exceções validados por pelo menos direção + coordenação + amostra de veterinários;
2. Inventário de ferramentas e materiais concluído (seção 6);
3. Público dimensionado e perfil segmentado (B-02);
4. Lacunas priorizadas por risco/frequência e áreas do piloto confirmadas (B-06);
5. Baseline mínima levantada (B-07);
6. Responsáveis nomeados (B-03);
7. Política de dados aprovada (B-05);
8. Situação de licenças verificada (B-04);
9. Mapa validado com os participantes (sessão de devolutiva);
10. Todos os formulários classificados (`FATO INFORMADO` / `HIPÓTESE` / `PENDENTE`) e nenhuma informação inventada.

**Resultado possível:**
- Todos os itens resolvidos → reexecutar primeiro o gate Discovery `0090`; somente se aprovado, atualizar e submeter o gate PRD `0090`;
- Algum item permanece aberto → o gate fica `BLOQUEADO` com a pendência registrada, e o PRD segue marcando os campos afetados como `PENDENTE`.

---

## 10. Checklist de logística

- [ ] Agendar entrevistas cobrindo os turnos da escala 12×36 (incluir finais de semana/plantões noturnos, se necessário);
- [ ] Enviar pauta prévia e garantir ambiente privado;
- [ ] Obter consentimento de registro (LGPD);
- [ ] Aplicar formulário individual;
- [ ] Consolidar e classificar respostas;
- [ ] Sessão de devolutiva com os participantes para validar o mapa do fluxo;
- [ ] Atualizar os documentos do briefing conforme seção 8;
- [ ] Reportar ao patrocinador e solicitar decisões pendentes;
- [ ] Registrar o resultado no log de execução (estado, próxima ação, bloqueios).
