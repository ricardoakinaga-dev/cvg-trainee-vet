# 0002 — Contexto Operacional

**Projeto:** Sistema CVG de Treinamento Veterinário  
**Status:** parcialmente conhecido; lacunas registradas

## Onde o problema ocorre

- No processo de integração, capacitação e desenvolvimento contínuo dos médicos-veterinários do CVG;
- Na relação entre treinamento teórico, avaliação e acompanhamento da evolução;
- Na governança de conteúdos e referências clínicas;
- Na gestão das evidências de aprendizagem.

### Abrangência operacional informada

- **Unidade física:** Centro Veterinário Guarapiranga;
- **Endereço:** Avenida Guarapiranga, 1993 — Vila Socorro, São Paulo/SP;
- **Setores:** Clínica Médica, Internação, Cirurgia, Laboratório de Análises Clínicas e Ultrassonografia;
- **Funcionamento:** atendimento 24 horas;
- **Regime de trabalho informado:** escala 12 × 36.

**CONFIRMADO — D-079:** o público é de aproximadamente 10 médicos-veterinários e todos participam da primeira aplicação. Setor, turno e tipo de vínculo não serão usados para segmentar o MVP.

## Etapas do fluxo afetadas

1. Entrada e diagnóstico amplo fracionado;
2. Recomendação da trilha;
3. Módulos digitais breves;
4. Quiz e caso/simulação digital;
5. Feedback imediato;
6. Progresso e revisão direcionada;
7. Atualização segura do conteúdo pela coordenação.

Os itens acima representam o **fluxo desejado declarado**, não o fluxo atual confirmado.

## Atores envolvidos

### Confirmado

- Médico-veterinário colaborador;
- Centro Veterinário Guarapiranga como organização patrocinadora;
- MV. Ricardo Akinaga como responsável pelas frentes do MVP;
- MV. Ricardo Akinaga como único aprovador clínico obrigatório; revisão adicional pode ser solicitada quando julgar necessário (D-083).

### Eventual apoio operacional

- eventual administrador técnico delegado, com acesso limitado e registrado.

## Ferramentas atuais — inventário institucional concluído

**FATO INFORMADO (D-078):** o CVG não possui sistema nem registro centralizado de treinamento. Ferramentas auxiliares eventualmente usadas em situações informais não formam um processo institucional.

No escopo institucional do treinamento, o inventário atual é: **nenhuma plataforma, banco de avaliações, trilha, registro de progresso ou integração dedicada**. As categorias auxiliares abaixo podem existir informalmente, mas não compõem processo institucional nem são dependência da SPEC:

- Armazenamento de materiais;
- Ferramentas de videoconferência;
- Planilhas;
- Sistemas de RH;
- Sistema de gestão clínica/prontuário;
- Ferramentas de comunicação;
- Processo de aplicação de provas;
- Forma de registro de certificados;
- Processo de revisão clínica.

## Processo atual

**FATO INFORMADO PELO RESPONSÁVEL — D-078:** o CVG não possui treinamento veterinário padronizado. O aprendizado ocorre informalmente, conforme a disponibilidade dos profissionais, sem trilha, avaliação ou registro centralizado.

B-01 está fechado com essa descrição. Detalhar conversas, materiais ou atividades isoladas é opcional e poderá apoiar melhorias futuras, mas não muda o diagnóstico institucional.

## Workaround existente

**FATO INFORMADO (D-078):** o aprendizado informal conforme a disponibilidade dos profissionais é o mecanismo existente. Ele não constitui trilha ou processo padronizado.

Exemplos de práticas que podem ocorrer, sem necessidade de inventário para fechar B-01:

- Conversas e discussões informais;
- Cursos externos;
- Reuniões clínicas;
- Materiais enviados por mensagem;
- Planilhas de presença;
- Leitura autônoma;
- Acompanhamento individual por especialistas.

## Restrições operacionais

- Cobertura de plantões e turnos;
- Tempo disponível para estudo;
- Dispositivos/conectividade: nenhuma restrição adicional foi informada; baseline de produto web responsiva em celular e computador;
- Acessibilidade: WCAG 2.2 AA aprovada em D-099 e acomodação operacional proposta em D-104;
- Uso simultâneo de uma equipe de aproximadamente 10 veterinários;
- Uso em celular e computador;
- Confidencialidade dos resultados;
- Separação entre desenvolvimento e decisão disciplinar;
- Casos clínicos: somente fictícios no MVP, conforme D-077;
- Dados de pacientes e tutores: proibidos no MVP, sem depender de anonimização.

## Contexto da fonte de conhecimento

O tratado brasileiro indicado:

- Abrange cães e gatos;
- Possui dois volumes, 23 partes e 264 capítulos;
- Foi publicado em 2015;
- Inclui medicina intensiva, dor, imunologia, nutrição, neonatologia, oncologia, toxicologia, doenças infecciosas e parasitárias, equilíbrio hidroeletrolítico e acidobásico, sistemas orgânicos, neurologia, responsabilidade profissional e medicina legal;
- Inclui advertência expressa sobre necessidade de consultar fontes atualizadas, sobretudo para terapêutica, doses, reações adversas e legislação;
- Possui direitos autorais que precisam ser respeitados.

O Ettinger indicado:

- Abrange medicina interna de cães e gatos;
- É a 9ª edição, publicada pela Elsevier em 2024;
- Possui dois volumes, 22 seções e 331 capítulos;
- O arquivo local possui 2.801 páginas;
- Inclui fundamentos de história clínica, exame físico, medicina baseada em evidências, comunicação, uso responsável de antimicrobianos, diagnósticos diferenciais, técnicas, emergência e especialidades;
- Também exige verificação independente de diagnósticos e doses;
- Tem proteção expressa de direitos autorais, inclusive para mineração de texto e treinamento de IA.

O Fossum indicado (fonte cirúrgica complementar — ver anexo 0001, F-03):

- Abrange cirurgia de pequenos animais (cães e gatos);
- É a 4ª edição, publicada pela Elsevier em 2014;
- Possui quatro partes (princípios cirúrgicos gerais, tecido mole, ortopedia, neurocirurgia) e 44 capítulos;
- O arquivo local possui 5.008 páginas;
- Também exige verificação independente de doses, protocolos e atualidade;
- Tem proteção expressa de direitos autorais.

Hierarquia proposta:

1. Legislação, regulação, bula e alertas vigentes;
2. Protocolos internos do CVG aprovados e atuais;
3. Diretrizes e consensos atuais aprovados pelo comitê;
4. Ettinger, 9ª edição, como referência clínica atualizada;
5. Fossum, 4ª edição, para temas cirúrgicos e perioperatórios;
6. Tratado brasileiro como base curricular em português e contextualização nacional.

## Dependências externas

- Licença/autorização para uso institucional e criação de material derivado das três obras;
- Fontes atualizadas para temas clínicos críticos;
- Protocolos internos do CVG;
- Disponibilidade de especialistas revisores;
- Regras do CFMV/CRMV e legislação aplicável;
- Política interna de dados e LGPD;
- Aprovação da direção e coordenação clínica.
