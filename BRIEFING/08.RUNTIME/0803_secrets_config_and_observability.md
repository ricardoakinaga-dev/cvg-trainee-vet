...
Logs JSON com `request_id`, `trace_id`, `correlation_id`, serviço, evento, status e duração. Métricas: disponibilidade, erro, p95/p99, PostgreSQL pool, outbox lag, retry, Qdrant index lag, IA latency/refusal e smoke. Traces atravessam web→API→worker→dependência quando a correlação técnica está disponível.
...
`@cvg/observability` fornece o logger JSON redigido, correlação e `traceId`
  validados, contadores/histogramas em memória e spans OTLP com somente IDs
  técnicos, rota, status e outcome. A API emite eventos por rota normalizada e
  o worker emite eventos de lote/resultado e spans por evento, sempre com
  allowlist e sem payload. O sink padrão escreve somente o registro já
  redigido.
...
O item 12 materializou o exporter interno Prometheus, health/dependencies redigido, avaliação de SLO/alertas e o contrato operacional 0804. O perfil local versiona Prometheus com retenção de 15 dias e Tempo local com `backend_worker.compaction.block_retention: 336h`; exportação externa, retenção efetiva do fornecedor, dashboards provisionados e RBAC de telemetria continuam dependentes do ambiente operacional.
