...
describe.skipIf(
  ...
  it("persists SaveAnswer atomically, redacts events, and authenticates server sessions", async () => {
    ...
    try {
      ...
      const storedAnswers = await adminDatabase.db
        .select()
        .from(answers)
      ...
      const storedAudit = await database.db.transaction(async (transaction) => {
      ...
      await adminDatabase.db.delete(accounts).where(eq(accounts.id, accountId));
