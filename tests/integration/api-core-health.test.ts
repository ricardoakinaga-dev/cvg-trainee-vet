...
describe.skipIf(!runLiveHealthTest || databaseUrl === undefined)(
  "API live core health endpoints",
  () => {
    it("reports PostgreSQL readiness and redacted disabled dependencies", async () => {
      if (databaseUrl === undefined) {
        throw new Error("live database configuration is required");
      }
      ...
      try {
        ...
        expect(JSON.stringify(body)).not.toMatch(
          /secret|password|api_key|url/iu,
        );
      } finally {
        await runtime.close();
      }
