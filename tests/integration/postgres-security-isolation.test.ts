...
describe.skipIf(
  ...
  it("requires transaction context and isolates participant and staff scope paths", async () => {
    ...
    const admin = createPostgresDatabase(adminDatabaseUrl);
    const api = createPostgresDatabase(databaseUrl, {
    ...
    try {
      await expect(api.healthcheck()).resolves.toBeUndefined();
      await expect(
        api.db.transaction(async (transaction) => {
          await transaction.execute(
            sql`select set_config('cvg.ai_suggestion_worker', 'true', true)`,
          );
          await transaction.select().from(aiSuggestions);
          await transaction.select().from(aiSuggestionEvents);
        }),
      ...
      await admin.db.execute(
        sql.raw(`grant select, update on accounts to ${role}`),
      );
      await admin.db.execute(
      ...
      await admin.db.execute(
        sql.raw(
          `grant delete on attempt_idempotency, answer_idempotency, assessment_idempotency to ${role}`,
        ),
      );
      await admin.db.execute(
      ...
      const restrictedUrl = new URL(adminDatabaseUrl);
      restrictedUrl.username = roleName;
      restrictedUrl.password = rolePassword;
      const restricted = createPostgresDatabase(restrictedUrl.toString());
      ...
      try {
        ...
        const privilegedGuard = createPostgresDatabase(adminDatabaseUrl, {
          requireLeastPrivilege: true,
        });
        try {
        ...
        const withoutContext = await restricted.db.transaction(
          async (transaction) => ({
            ...
            runtimes: await transaction.select().from(curriculumRuntimeStates),
        ...
        await expect(
          journeyRepository.findParticipantLearningJourney(otherParticipantId, [
            scopeId,
          ]),
      ...
      if (attemptId !== null) {
        ...
        await admin.db.delete(answers).where(eq(answers.attemptId, attemptId));
        await admin.db
