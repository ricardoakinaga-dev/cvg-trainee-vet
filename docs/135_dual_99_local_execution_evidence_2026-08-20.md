...
- corte: `2026-08-21T07:59:41-03:00`
- última atualização: `2026-08-21T07:59:41-03:00`
...
- a auditoria read-only encontrou que arquivos staged maiores que o cap por
...
- GREEN desconta `MAX_SCAN_BYTES + 1` em cada output-cap abortado e falha
  fechado ao esgotar `256 MiB` da superfície staged, sem findings parciais;
...
- a regressão final usa `129 × 2 MiB` (`270532608` bytes), foco `63/63`,
  ...
  passaram. O código/teste `48e1014` e a documentação inicial `abb1657` foram
  publicados; a reconciliação final desta rodada não altera código ou estado
  externo.
...
- foco `62/62`, cobertura `205/1158/21` em `95,03/90,95/95,31/95,73`, build
  ...
  código/teste `938bc41` e a documentação inicial `0b5ea42` foram publicados;
  a reconciliação final desta rodada não altera código ou estado externo.
...
- a auditoria read-only encontrou que o cap por diretório permitia atravessar
  uma árvore distribuída com `2048` diretórios e `2048` arquivos; o RED focal
  confirmou a ausência do fallback genérico;
- GREEN propaga um orçamento global imutável de `4096` entradas pela recursão.
  Ao esgotar, a travessia inteira falha fechado e retorna
...
- foco `60/60`, cobertura `205/1156/21` em `95,03/90,95/95,31/95,73`, build
  ...
  genérico. O código/teste `e59d88c` e a documentação inicial `4ba2a70` foram
  publicados; a reconciliação final desta rodada não altera código ou estado
  externo.

## Round 65 — limite de entradas do worktree

- a auditoria read-only encontrou que a enumeração do worktree materializava
  `2000` arquivos sintéticos porque `readdir` não tinha orçamento; o RED focal
  confirmou que o scan retornava lista vazia em vez do fallback genérico;
- GREEN separa a abertura no-follow da enumeração, usa `opendir` incremental e
  limita cada diretório a `1024` entradas. Overflow fecha a travessia e retorna
  o finding redigido `<workspace> / unreadable-file`;
...
- foco `58/58`, cobertura `205/1154/21` em `95,03/90,95/95,31/95,73`, build
  ...
  genérico. O código/teste `ace0054` e a documentação inicial `f63547e` foram
  publicados; a reconciliação final desta rodada não altera código ou estado
  externo.
...
alterado. O commit de código/teste `c69069b` e a reconciliação documental
`13f64f1` foram publicados no branch remoto; o pós-push confirmou
`HEAD == origin` em `13f64f1`. A
